...
function initSearch(rawSearchIndex) {
    ...
    function execQuery(parsedQuery, searchWords, filterCrates, currentCrate) {
        ...
        function addIntoResults(results, fullId, id, index, lev, maxLevDistance) {
            if (lev === 0 || (!parsedQuery.literalSearch && lev <= maxLevDistance) || index > -1) {
        ...
        function handleSingleArg(
            ...
            addIntoResults(results_in_args, fullId, pos, -1, in_args, maxLevDistance);
            addIntoResults(results_returned, fullId, pos, -1, returned, maxLevDistance);
            ...
            if (elem.name.length === 0) {
                if (row.type !== null) {
                    ...
                    addIntoResults(results_others, fullId, pos, -1, lev, maxLevDistance);
                }
                return;
            }

            index = searchWord.indexOf(elem.pathLast);
            if (index === -1) {
            ...
            lev = levenshtein(searchWord, elem.pathLast) + lev_add;
            if (lev > maxLevDistance && index === -1) {
                return;
            }
            if (lev < 0) {
                lev = 0;
        ...
        function handleArgs(row, pos, results, maxLevDistance) {
            ...
            function checkArgs(elems, callback) {
                for (const elem of elems) {
                    ...
                    let index = row.normalizedName.indexOf(elem.pathLast);
            ...
            addIntoResults(results, row.id, pos, minIndex, lev, maxLevDistance);
        }
